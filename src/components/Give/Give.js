import classes from './Give.module.css'
...
import SingleProduct from '../Profile/SingleProduct/SingleProduct';
import SingleTake from '../Profile/SingleTake/SingleTake';
...
const Give = () => {
    ...
    let dropDownState = state_arr.map((item, idx) => {
        return {
            key: item, text: item, value: item, idx:idx,
    ...
    if (regionState.state) {
        let currentState = dropDownState.filter((currentItm) => {
            return currentItm.text === regionState.state
        })
        currentState = currentState[0]
        const idx = currentState.idx + 1; 
        let cities = s_a[idx].trim().split('|');
        dropdownCity = cities.map((item, idx) => {
            return {
                key: item, text: item, value: item, idx: idx, label: item,
    ...
    const buttonHandler = () => {
        if(nameRef.current.value){
            if(!(nameRef.current.value.length >= 3) ){
                changeError("Product Name Field Should Be Atleast 3 Characters Long ")
            }else {
                if(quantityRef.current.value){
                    if(parseInt(quantityRef.current.value) <= 0){
                        changeError("Quantity cannot be less than 1")
                    }else {
                        if(descRef.current.value){
                            if(descRef.current.value.length <= 3){
                                changeError("Give Full Description of the product")
                            }else{
                                console.log(image)
                                if(image.length <= 0 ){
                                    
                                    changeError("You have to add atleast one image")
                                }else {
                                    if (!(regionState.state)){
                                        changeError("You have to add a state for pickup")
                                    }else {
                                        if(!(regionState.city)){
                                            changeError("You have to add a city for pickup")
                                        }else {
                                            if (addressRef.current.value){
                                                if(!(addressRef.current.value.length > 3)){
                                                    changeError("Enter the full Address")
                                                }else{
                                                    let data = {};
                                                    data.name = nameRef.current.value;
                                                    data.quantity = quantityRef.current.value;
                                                    data.desc = descRef.current.value;
                                                    data.image = image.slice(0,5);
                                                    data.state = regionState.state;
                                                    data.city = regionState.city;
                                                    data.address1 = addressRef.current.value;
                                                    data.address2 = addressRef2.current.value; 
                                                    data.user = userState._id;
                                                    changeLoading(true)
                                                ...
                                                changeError("Address Line needs to be entered")
                                            }
                                        }
                                    }
                                }
                            ...
                            changeError("Description Field Cannot Be Empty")
                        }
                    }
                }else {
                    changeError("Quantity Field Cannot Be Empty")
                }
            }
        }else{
            changeError("Product Name needs to be entered")
        }
    }
    ...
    const changeHandler = (event) => {
        ...
        const array = []
        const FileLoop = (index) => {
             fileIntoPromise(files[index]).then((item)=>{
                 array.push(item);
                 index++;
    ...
    return (
        ...
        {!bigLoading &&
        <> 
                <div className={add ? classes.giveDivScrollable : classes.giveDiv}>
                    ...
                    <div className={classes.addBtn}>
                        ...
                        <button onClick={givenProductsHandler}> 
                            {add === 'give' && "Hide Given Products"}
                            {add !== 'give' && "Total Given Products"}
                        </button>
                        <button onClick={takeProductsHandler}> 
                            {add === 'take' && "Hide Take Requests"}
                            {add !== 'take' && "Your Take Requests"}
                        </button>
                    </div>
                    {add === 'add' && 
                    <>
                        ...
                        <div className={classes.formDiv}>
                            ...
                            <form className={classes.formDiv1}>
                                {error && 
                                <Flip top>
                                    <Alert>{error}</Alert>
                                </Flip>}
                            <Form>
                                ...
                                <Form.Group className="mb-3" controlId="formBasicPassword">
                                    ...
                                    <Form.Control ref={quantityRef} type="number" placeholder="Enter Product Quantity" />
                                </Form.Group>
                                <Form.Group className='mb-3'>
                                    <Form.Label>Product Description</Form.Label>
                                    <Form.Control ref={descRef} type="text" placeholder="Enter Product Description" />
                                </Form.Group>
                                <Form.Group className='mb-3'>
                                    <Form.Label>Product Price</Form.Label>
                                    <Form.Control type='text' placeholder='0' readOnly></Form.Control>
                                </Form.Group>
                                <Form.Group className="mb-3" controlId="formBasicImage">
                                    <Form.Label>Product Image</Form.Label>
                                    <Form.Control ref={imageRef} type="file" onChange={changeHandler} multiple placeholder="Enter Product Image" />
                                </Form.Group>
                                <div className={classes.imageDiv}>
                                    {image.map((singleItem)=>{
                                        return (
                                            <div>
                                            <Flip top cascade>
                                ...
                                <Form.Group className='mb-3'>
                                    ...
                                    <Select isSearchable={true} styles={CustomStyles} onChange={(data)=>{ changeRegionState({state: data.value})}} options={dropDownState} placeholder="Enter State Name"></Select>
                                </Form.Group>
                                {regionState.state && 
                                <Form.Group className='mb-3'>
                                    <Form.Label>City Name</Form.Label>
                                    <Select isSearchable={true} styles={CustomStyles} onChange={(data)=>{
                                        let obj = regionState;
                                        obj.city = data.value;
                                        changeRegionState(obj);
                                    }} options={dropdownCity}></Select>
                                </Form.Group>}
                                <Form.Group className={'mb-3'}>
                                    <Form.Label>Pickup Address Line 1</Form.Label>
                                    <Form.Control ref={addressRef} placeholder={'Enter The Address Line #1'}></Form.Control>
                                </Form.Group>
                                <Form.Group className={'mb-3'}>
                                    <Form.Label>Pickup Address Line 2</Form.Label>
                                    <Form.Control ref={addressRef2} placeholder={'Enter The Address Line #2'}></Form.Control>
                                </Form.Group>
                                <Button onClick={buttonHandler} variant="light">
